...
<button
  class="
    flex items-center ml-auto px-3 md:px-5 py-2 md:py-1 rounded-lg  text-white
  ...
  on:click={openDialog}
>
  <span class="hidden md:inline font-body mr-10 text-lg">Keresés...</span>
  <SearchIcon class="block w-4 h-4 invert" />
</button>

{#if dialogOpen}
  <Portal target="body">
    <SearchDialog on:close={closeDialog} />
  </Portal>
{/if}
